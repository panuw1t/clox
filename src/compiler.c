...
static const char* PrecedenceNames[] = {
  "PREC_NONE",
  "PREC_ASSIGNMENT",
  "PREC_OR",
  "PREC_AND",
...
static void ternary(bool canAssign) {
  TRACE_ENTRY();
  parsePrecedence(PREC_CONDITION);
...
static int switchCase(int exit) {
  TRACE_ENTRY();
  emitByte(OP_DUPE);
  expression();
  emitByte(OP_EQUAL);
...
static void switchStatement() {
  ...
  if (match(TOKEN_DEFAULT)) {
    consume(TOKEN_COLON, "Expect ':' after expression.");
    int dejump = emitJump(OP_JUMP);
    defau = currentChunk()->count;
    while (!check(TOKEN_RIGHT_BRACE) &&
           !check(TOKEN_EOF) &&
           !check(TOKEN_CASE)) {
      statement();
...
ParseRule rules[] = {
  ...
  [TOKEN_QUESTION]      = {NULL,     ternary,PREC_CONDITION},
};

static void parsePrecedence(Precedence precedence) {
  TRACE_ENTRY("(precedence=%s)", PrecedenceNames[precedence]);
  advance();
